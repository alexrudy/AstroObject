    ...
    def decorator(func):
        def clip_traceback(*args,**kwargs):
            try:
                ...
                if not __libdebug__:
                    #####################################
                    ### SEE PREVIOUS FRAME FOR ERROR
                    raise exc # TRACEBACK HIDDEN
                    ### EXCEPTION GENERATED BY AO Library
                    #####################################
                else:
                    # We want to raise the original exception, because we are in normal AO mode.
                    raise
        make_decorator(func)(clip_traceback)
