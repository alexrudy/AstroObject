...
#the version number - this file is only used in isdev mode or during build.
...
release = not isdev
_warntxt = ', so no revision number available for isdev build of AstroObject - this may prevent this install from overwriting the previous version'
...
isgitdev = _get_git_devstatus()
release = not ( isdev or isgitdev )
...
version = str(major)+'.' +\
          ...
          ( ('-' + devstr )            if ( devstr is not None and isdev ) else '' ) +\
          ( ('-' + _get_devstr(True) ) if isgitdev else '')
          
_frozen_version_py_template = """#Autogenerated in AstroObject setup.py on %s
#these components can be changed for new versions
version = '%s'
